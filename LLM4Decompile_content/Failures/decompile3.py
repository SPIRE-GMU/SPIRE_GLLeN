...
inputs = tokenizer(asm_func, return_tensors="pt").to(
    "cuda:0"
)  # Ensure the inputs are on the same device as DataParallel's default

# Generate output without gradient computation
with torch.no_grad():
    outputs = model.module.generate(
        **inputs, max_new_tokens=2048
    )  # Use model.module to access generate method
