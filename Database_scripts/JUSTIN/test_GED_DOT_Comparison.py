...
def flatten_pydot(dot: str) -> pydot.Dot:
    ...
    if not graphs: raise ValueError("empty dot")
    root = pydot.Dot(graph_type="digraph")
    def rec(g):
        for n in g.get_nodes():
            if n.get_name() not in {"node","graph","edge"}:
                root.add_node(n)
        for e in g.get_edges(): root.add_edge(e)
        for sg in g.get_subgraphs(): rec(sg)
    for g in graphs: rec(g)
    return root
...
def load_dot(path:str) -> nx.DiGraph:
    ...
    H=nx.DiGraph()
    for n in G: H.add_node(n.split(":")[0])
    for u,v in G.edges(): H.add_edge(u.split(":")[0], v.split(":")[0])
    return H
...
def radare_expand(G, dbg=False):
    H=G.copy(); added=[]; sid=0
    for u,v in list(H.edges()):
        if H.out_degree(u)>=2 and H.out_degree(v)==0:
            s=f"SYNTH_{sid}"; sid+=1
            H.remove_edge(u,v); H.add_node(s)
            H.add_edge(u,s); H.add_edge(s,v); added.append(s)
    if dbg and added: print("[smart‑expand] inserted:", added)
    return H

# ───────────────────────── pretty dump ────────────────────────
def dump(G,name):
    print(f"[{name}] Nodes ({G.number_of_nodes()}):")
    for n in sorted(G): print(f"    {n}")
    print(f"[{name}] Edges ({G.number_of_edges()}):")
    for u,v in sorted(G.edges()): print(f"    {u} -> {v}")
...
ap.add_argument("--smart",choices=["radare","none"],default="none",
...
print(f"{args.radare_dot.split('/')[-1]}: |V|={s1['V']} |E|={s1['E']} loops={s1['loops']} decisions={s1['dec']}")
print(f"{args.gcc_dot.split('/')[-1]}: |V|={s2['V']} |E|={s2['E']} loops={s2['loops']} decisions={s2['dec']}\n")
