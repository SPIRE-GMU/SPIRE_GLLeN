    ...
    with driver.session() as session:
        for file_path, function_data in cfg_data.items():
            ...
            session.run(
                ...
                SET f.function_name = $function_name, f.full_code = $full_code, f.name = $function_name
                """,
            ...
            for block_id, block_data in function_data['basic_blocks'].items():
                session.run(
                    ...
                    SET b.block_code = $block_code, b.contains_decision = $contains_decision,
                        b.is_loop_header = $is_loop_header, b.name = 'BB-' + split($block_id, '-')[3]
                    """,
            ...
            starting_block = next((block_id for block_id, block_data in function_data['basic_blocks'].items() if block_id.endswith('-bb-2')), None)
            if starting_block:
                session.run(
                    """
            ...
            for loop_data in function_data['loops']:
                ...
                session.run(
                    ...
                    SET l.header = $header, l.latch = $latch, l.depth = $depth, l.name = 'Loop-' + split($loop_id, '-')[3]
                    """,
            ...
            for decision_data in function_data['decision_nodes']:
                session.run(
                    ...
                    SET d.condition = $condition, d.name = $condition
                    """,
