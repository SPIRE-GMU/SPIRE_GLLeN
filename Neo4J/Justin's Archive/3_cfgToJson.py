...
def extract_cfg_elements(file_content, function_prefix):
    ...
    for line in file_content:
        ...
        # Extract basic block instructions and identify if statements
            ...
            if "if" in line:
                ...
                decision_pending = (
                    decision  # Keep track of the decision to set targets later
                )
        ...
        # Extract successors (edges between nodes)
            ...
            if match:
                ...
                if (
                    decision_pending
                    and decision_pending["true_target"]
                    and decision_pending["false_target"]
                ):
                    decision_pending = None
    ...
    for loop in data["loops"]:
        if loop["latch"] is not None:
            ...
            if (
                latch_block_uuid in data["basic_blocks"]
                and header_block_uuid in data["basic_blocks"]
            ):
                data["edges"].append(
                    {"from": latch_block_uuid, "to": header_block_uuid}
                )
                data["basic_blocks"][latch_block_uuid]["successors"].append(
                    header_block_uuid
                )
