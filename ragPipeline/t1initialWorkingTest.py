...
def embed_code(code_snippet):
    input = tokenizer(code_snippet, return_tensors="pt",truncation=True,max_length=128)
    outputs = model(**input)
...
def generate_embeddings():
    ...
    assembly_embeddings = np.array([embed_code(assembly_code) for assembly_code in assembly_code_list])
...
asm = ["factorial.s", "fibonacci.s", "floating_mult.s", "hello_world.s", "natural_sum_recursion.s", "num_swap.s",
        "powers_loop.s", "quadratic_roots.s", "quotient_remainder.s", "recursive_factorial.s"]

c = ["factorial.c", "fibonacci.c", "floating_mult.c", "hello_world.c", "natural_sum_recursion.c", "num_swap.c",
        "powers_loop.c", "quadratic_roots.c", "quotient_remainder.c", "recursive_factorial.c"]
