...
def preprocess_and_decompile(file_name, model_option):
    ...
    for opt_state in OPT:
        ...
        try:
            ...
            inputs = tokenizer(asm_func, return_tensors="pt").to(next(model.parameters()).device)
            with torch.no_grad():
                # Call the generate method on the underlying model
                outputs = model.module.generate(**inputs, max_new_tokens=2048)

            if outputs.size(1) > len(inputs['input_ids']):
                c_func_decompile = tokenizer.decode(outputs[0][len(inputs['input_ids']):-1])
            else:
                c_func_decompile = ""
