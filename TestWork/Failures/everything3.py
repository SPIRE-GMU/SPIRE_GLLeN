        ...
        try:
            ...
            with open(f"{output_file}.s") as f:
                ...
                for line in asm.split("\n"):
                    ...
                    tmp_asm = "\t".join(line.split("\t")[idx:])  # Remove the binary code
            ...
            print(f"Allocated memory before loading model: {torch.cuda.memory_allocated()} bytes")
            print(f"Cached memory before loading model: {torch.cuda.memory_reserved()} bytes")

            print(f'Model parameters before prepare: {sum(p.numel() for p in model.parameters())}')

            # Prepare model for distributed training
            model, inputs = accelerator.prepare(model, tokenizer(input_asm_prompt, return_tensors="pt", truncation=True, padding=True))

            print(f'Model parameters after prepare: {sum(p.numel() for p in model.parameters())}')
            print(f"Allocated memory after loading model: {torch.cuda.memory_allocated()} bytes")
            print(f"Cached memory after loading model: {torch.cuda.memory_reserved()} bytes")

            with open(f"{output_file}.asm", 'r') as f:  # Read the assembly
                asm_func = f.read()
                
            # Tokenize the assembly function, only necessary parts
            inputs = tokenizer(asm_func, return_tensors="pt", truncation=True, padding=True)

            # Move inputs to the same device as the model
            inputs = inputs.to(accelerator.device)  # Ensure inputs are on the same device (GPU)
